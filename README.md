...
Articles are written in [MDX](https://mdxjs.com/) and validated via [Velite](https://velite.js.org/),
which runs as its own build step so a Next.js upgrade cannot break the content pipeline. Images live in `public/` and are served through `next/image`; run `pnpm images:optimize` after adding large pictures to shrink them.
...
Image sizes are read at build time and injected into the MDX, so pictures inside an article
go through `next/image` automatically. Animated files are detected and left untouched.
